    ...
    var rewriteRulesSnippet = require('./node_modules/grunt-connect-route/lib/utils').rewriteRequest,
        ...
        getModulesConfig = function(dir) {
            ...
            grunt.file.recurse(dir, function(abspath, rootdir, subdir, filename) {
                if (filename === 'main.js') {
                    returnConfig.push({
                        name: 'pages/' + (subdir ? subdir + '/' : '') + filename.replace('.js', ''),
            ...
            requirejs: {
                dist: {
                    ...
                    options: {
                        ...
                        baseUrl: '.',
                        paths: require('./app/static/js/require-config').paths,
                        shim: require('./app/static/js/require-config').shim,
                        dir: '<%=path.dist%>/static/js', //build dir
                        optimize: 'none',
                        findNestedDependencies: true,
                        inlineText: true,
                        removeCombined: true,
                        modules: getModulesConfig('app/static/js/widget')
                    }
                }
            },
